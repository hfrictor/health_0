    ...
    def WriteValue(self, value, options):
        ...
        if value == '':
            ...
            if len(self.alls) == 2:
                ...
                print(type(temp))
                print("wpa_supplicant written")
                self.alls = []
                print(ip_address)
                ps = subprocess.Popen(['iwconfig'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                try:
                    self.value = subprocess.check_output(('grep', 'ESSID'), stdin=ps.stdout)
                    print(self.value)
                except subprocess.CalledProcessError:
                    # grep did not match any lines
                    self.value = "No wireless networks connected"
